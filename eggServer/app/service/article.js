...
class ArticleService extends Service {
  ...
  async getContentById(id) {
    try {
      ...
      //
      const resultArticle = await Article.findOne({ _id: id }).exec();
      // thumbUpNumber
      let num = resultArticle.readNumber;
      num += 1;
      console.log('readNumber::', resultArticle.readNumber, 'num::', num, resultArticle.id);
      const update = await Article.update({ id: resultArticle.id }, { $set: { readNumber: num } });
      const reuslt = await Article.findOne({ _id: id }).exec();
  ...
  async zhan(id) {
    try {
      if (id) {
        ...
        num += 1;
        const update = await Article.update({ _id: resultArticle._id }, { $set: { commentNumber: num } });
  ...
  async comment(articleId, name, email, url, desc) {
    try {
      ...
      let comment = [],
        commentItem = [],
        result = [];
      const list = await Comment.findOne({ articleId });
      ...
      if (list) {
        const newarr = list.comment.concat(comment);
        result = await Comment.update({ articleId }, { $set: { comment: newarr } });
      } else {
        commentItem = new Comment({ articleId, comment });
  ...
  */
  async getCommentByArticleId(articleId) {
    try {
      ...
      const Comment = this.ctx.model.Comment;
      const result = await Comment.find({ articleId });
