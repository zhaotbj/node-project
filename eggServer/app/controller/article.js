...
class ArticleController extends Controller {
  ...
  async create(){
    ...
    let {userId, userName, content, title,category, readNumber, commentNumber, thumbUpNumber} = this.ctx.request.body;
  ...
  async update(){
    ...
    let articleId = this.ctx.request.body.id;
    if(!articleId) return
    // 执行更新数据
    let {content, title, image, summary, readNumber, commentNumber, thumbUpNumber } = this.ctx.request.body;
    let obj = {
      content: content,
      title: title,
      image: image,
